        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title"><b>Date de creation:</b> {{ $BonRetourZone->created_at }}</h5>
                </div>
            </div>
        </div>
    </div>
...
<div class="container-fluid mt-4">
  <form action="{{ route('bon.retour.zone.update.all',$BonRetourZone->id_BRZ) }}" method="POST">
  @csrf
  <div class="card">
    <div class="card-header">
      <h5><b>List des nouveaux Bons</b></h5>
    </div>
    <div class="card-body">
        ...
        <tbody>
          ...
          <tr id="new-parcel-Autedelenitidelect" role="row" class="odd">
            ...
            <td>
              ...
              <a href="{{ route('bon.retour.zone.update',['id'=>$item->id,'id_BRZ'=>$BonRetourZone->id_BRZ]) }}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i></a>
            </td>
          </tr>
          @endforeach
...
<div class="container-fluid mt-4">
  <form action="{{ route('bon.retour.zone.updateDelete.all',$BonRetourZone->id_BRZ) }}" method="POST">
  @csrf
  <div class="card">
    <div class="card-header">
        ...
        <tbody>
          ...
          @foreach ($colisBon as $i=>$item )
            ...
            <td><input class="table-delete-checkbox" name="colisDelete[{{ $i }}]" value="{{ $item->id }}" type="checkbox" ></td>
              ...
              <td>
                <a href="{{ route('bon.retour.zone.updateDelete',['id'=>$item->id,'id_BRZ'=>$BonRetourZone->id_BRZ]) }}" class="btn btn-sm btn-danger"><i class="fa fa-times"></i></a>
              </td>
            </tr>
          @endforeach
        </tbody>
