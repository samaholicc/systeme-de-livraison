    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <a href="{{ route('bon.retour.zone.create') }}" class="btn btn-primary">Ajouter Bon Retour Zone</a>
            </div>
        </div>
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach ($bons as $index => $item)
                        <tr>
                            ...
                            <td class="pe-0">
                                <span class="fw-bold"
                                    data-kt-ecommerce-product-filter="nomcomplet">{{ $item->zone->zonename }}</span>
                            </td>
                            ...
                            <td>
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-250px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a class="btn" onclick="openModal('{{ $item->id_BRZ }}')"
                                            data-bs-toggle="modal" data-bs-target="#kt_modal_new_target"><i
                                                class="fa fa-eye"></i>Details du bon</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a class="btn"
                                            href="{{ route('bon.retour.zone.exportColis', $item->id_BRZ) }}"><i
                                                class="far fa-file-excel"></i>Exporter les colis</a>
                                    </div>
                                    @if ($item->status != 'Recu')
                                        <div class="menu-item px-3">
                                            <form action="{{ route('bon.retour.zone.recu', $item->id_BRZ) }}"
                                                method="post">
                                                @csrf
                                    ...
                                    @else
                                        <div class="menu-item px-3">
                                            <form action="{{ route('bon.retour.zone.nonrecu', $item->id_BRZ) }}"
                                                method="post">
                                                @csrf
                                                <button type="submit"class="btn">
                                    ...
                                    @endif
                                    <div class="menu-item px-3">
                                        <a class="btn" href="{{ route('bon.retour.zone.getPdf', $item->id_BRZ) }}"><i
                                                class="far fa-file-pdf"></i>Voir en Pdf</a>
                                    </div>
                                    {{-- @if ($item->colis->count() == 0)
                                        <div class="menu-item  text-hover-danger px-3">
                                            <a href="{{ route('bon.retour.zone.destroy', $item->id_BRZ) }}"
                                                class="btn">
                                    ...
                                    @if ($item->colis->count() == 0)
  <div class="menu-item text-hover-danger px-3">
    <form action="{{ route('bon.retour.zone.destroy', $item->id_BRZ) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
      @csrf
      @method('DELETE')
      <button type="submit" class="btn">
        <i class="fa fa-trash"></i> Delete
...
@endif
    ...
    <script>
        ...
        function openModal(id) {
            ...
            let bons = @json($bons);
            console.log(bons);
            let BD = bons.find(ele => ele.id_BRZ == id)
            console.log(BD);
            let text = ''
            text = `<div class="row">
    <div class="dn-inv-infos-box col-6">
      <b>Bon d envoie : </b> ${BD.id_BRZ}<br>
