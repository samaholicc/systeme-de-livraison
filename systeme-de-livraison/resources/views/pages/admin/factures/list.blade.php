...
@section('content')
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach ($bons as $index => $item)
                        <tr>
                            ...
                            <td class="pe-0">
                                <span class="fw-bold"
                                    data-kt-ecommerce-product-filter="nomcomplet">{{ $item->nomcomplet }}</span>
                            </td>
                            ...
                            <td>
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-250px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a class="btn" onclick="openModal('{{ $item->id_F }}')"
                                            data-bs-toggle="modal" data-bs-target="#kt_modal_new_target"><i
                                                class="fa fa-eye"></i>Details du bon</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a class="btn" href="{{ route('factures.exportColis', $item->id_F) }}"><i
                                        ...
                                        <div class="menu-item px-3">
                                            <form action="{{ route('factures.recu', $item->id_F) }}" method="post">
                                                ...
                                                <button type="submit"class="btn">
                                                    <i class="fa fa-check"></i>Enregistre Facture
                                                </button>
                                            </form>
                                        </div>
                                    @elseif ($item->status == 'Enregistre')
                                        <div class="menu-item px-3">
                                            <div class="menu-item px-3">
                                                <form action="{{ route('factures.paye', $item->id_F) }}" method="post">
                                                    @csrf
                                                    <button type="submit"class="btn">
                                                        <i class="fa fa-check"></i>Paye Facture
                                                    </button>
                                                </form>
                                            </div>
                                    @endif
                                    <div class="menu-item px-3">
                                        <a class="btn" href="{{ route('factures.getPdf', $item->id_F) }}"><i
                ...
                @endif --}}
                                    @if ($item->colis->count() == 0)
                                        <div class="menu-item text-hover-danger px-3">
                                            <form action="{{ route('factures.destroy', $item->id_F) }}" method="POST"
    ...
    <script>
        ...
        function openModal(id) {
            ...
            let bons = @json($bons);
            console.log(bons);
            let BD = bons.find(ele => ele.id_F == id)
            console.log(BD);
            let text = ''
            text = `<div class="row">
    <div class="dn-inv-infos-box col-6">
      <b>Bon d envoie : </b> ${BD.id_F}<br>
