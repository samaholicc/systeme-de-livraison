            ...
            <td>
              ...
              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-250px py-4" data-kt-menu="true">
                <div class="menu-item px-3">
                  <a  class="btn" onclick="openModal('{{ $item->id_BPZ }}')"  data-bs-toggle="modal" data-bs-target="#kt_modal_new_target"><i class="fa fa-eye"></i>Details du bon</a>
                  
                </div>
                <div class="menu-item px-3">
                  <a  class="btn"  href="{{ route('bon.payment.zone.exportColis',$item->id_BPZ) }}"><i class="far fa-file-excel"></i>Exporter les colis</a>
                </div>
                <div class="menu-item px-3">
                  @if ($item->status != 'Paye')
                    <div class="menu-item px-3">
                  <form action="{{route('bon.payment.zone.recu',$item->id_BPZ)}}" method="post">
                    @csrf
                ...
                @if ($item->colis->count() == 0)
  <div class="menu-item text-hover-danger px-3">
    <form action="{{ route('bon.payment.zone.destroy', $item->id_BPZ) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
      @csrf
      @method('DELETE')
      <button type="submit" class="btn">
        <i class="fa fa-trash"></i> Delete
...
function openModal(id) {
  ...
  let bons=@json($bons);
  console.log(bons);
  let BE=bons.find(ele=>ele.id_BPZ==id)
  console.log(BE);
  let text=''
  text= `<div class="row">
    <div class="dn-inv-infos-box col-6">
      <b>Bon d envoie : </b> ${BE.id_BPZ}<br>
