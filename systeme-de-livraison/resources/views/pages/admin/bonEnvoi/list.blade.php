                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach ($bons as $index => $item)
                        <tr>
                                ...
                                <div class="">
                                    <div class="ms-5">
                                        <a href="" class="text-gray-800 text-hover-primary fs-5 fw-bold"
                                            data-kt-ecommerce-product-filter="code">{{ $item->id_BE }}</a>
                                    </div>
                                </div>
                            </td>
                            <td class="pe-0">
                            ...
                            <td class="pe-0">
                                <span class="fw-bold"
                                    data-kt-ecommerce-product-filter="ville">{{ $item->colis_count }}</span>
                            </td>
                            <td class="pe-0">
                                <span class="fw-bold"
                                    data-kt-ecommerce-product-filter="prix">{{ $item->colis_sum_prix }}</span>
                            </td>
                            <td>
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-250px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a class="btn" onclick="openModal('{{ $item->id_BE }}')"
                                            data-bs-toggle="modal" data-bs-target="#kt_modal_new_target"><i
                                                class="fa fa-eye"></i>Details du bon</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a class="btn" href="{{ route('bon.envoi.exportColis', $item->id_BE) }}"><i
                                    ...
                                    @if ($item->colis->count() == 0)
  <div class="menu-item text-hover-danger px-3">
    <form action="{{ route('bon.envoi.destroy', $item->id_BE) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
      @csrf
      @method('DELETE')
      <button type="submit" class="btn">
        <i class="fa fa-trash"></i> Delete
...
@endif
                                    ...
                                    <div class="menu-item px-3">
                                        <a class="btn" href="{{ route('bon.envoi.getPdf', $item->id_BE) }}"><i
    ...
    <script>
        ...
        function openModal(id) {
            ...
            let bons = @json($bons);
            console.log(bons);
            let BE = bons.find(ele => ele.id_BE == id)
            console.log(BE);
            let text = ''
            text = `<div class="row">
    <div class="dn-inv-infos-box col-6">
      <b>Bon d envoie : </b> ${BE.id_BE}<br>
      <b>Date :</b>${BE.created_at}<br>
      <b>Colis :</b> ${BE.colis_count}<br>
      <b>Total :</b> ${BE.colis_sum_prix} Dhs
    </div>
  </div>
